...
public class OperateLogAspect {
    ...
    private static void fillModuleFields(OperateLog operateLogObj,
        ...
        if (StrUtil.isEmpty(operateLogObj.getModule())) {
            Tag api = getClassAnnotation(joinPoint, Tag.class);
            if (api != null) {
                // 优先读取 @API 的 name 属性
                if (StrUtil.isNotEmpty(api.description())) {
                    operateLogObj.setModule(api.description());
                }
                // 没有的话，读取 @API 的 tags 属性
                if (StrUtil.isEmpty(operateLogObj.getModule()) && ArrayUtil.isNotEmpty(api.description())) {
                    operateLogObj.setModule(api.description());
                }
            }
        }
