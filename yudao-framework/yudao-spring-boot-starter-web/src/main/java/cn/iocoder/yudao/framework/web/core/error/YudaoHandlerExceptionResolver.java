...
@Slf4j
public class YudaoHandlerExceptionResolver extends DefaultHandlerExceptionResolver {
    @Override
    public int getOrder() {
    ...
    public ModelAndView doResolveException(
        ...
        try {
            if (ex instanceof ServiceException) {
                return handleServiceException((ServiceException) ex, request, response, handler);
            }
    ...
    protected ModelAndView handleServiceException(
            @NonNull ServiceException ex, @NonNull HttpServletRequest request,
        ...
        // ServiceException 因为是自定义的异常类，这个异常表示的是请求是正常的，只是业务失败，所以要将状态处理成200。然后报业务错误即可。
