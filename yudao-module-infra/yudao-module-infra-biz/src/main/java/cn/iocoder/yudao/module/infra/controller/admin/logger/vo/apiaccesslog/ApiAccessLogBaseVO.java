...
public class ApiAccessLogBaseVO {
    ...
    @Schema(title = "开始请求时间", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "开始请求时间不能为空")
    @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND)
    private LocalDateTime beginTime;

    @Schema(title = "结束请求时间", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "结束请求时间不能为空")
    @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND)
    private LocalDateTime endTime;

    @Schema(title = "执行时长", requiredMode = Schema.RequiredMode.REQUIRED, example = "100")
    @NotNull(message = "执行时长不能为空")
    private Integer duration;

    @Schema(title = "结果码", requiredMode = Schema.RequiredMode.REQUIRED, example = "0")
