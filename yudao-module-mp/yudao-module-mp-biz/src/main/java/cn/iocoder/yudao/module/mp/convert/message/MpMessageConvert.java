...
public interface MpMessageConvert {
    ...
    default WxMpXmlOutMessage convert02(MpMessageDO message, MpAccountDO account) {
        ...
        switch (message.getType()) {
            case WxConsts.XmlMsgType.TEXT -> builder = WxMpXmlOutMessage.TEXT().content(message.getContent());
            case WxConsts.XmlMsgType.IMAGE -> builder = WxMpXmlOutMessage.IMAGE().mediaId(message.getMediaId());
            case WxConsts.XmlMsgType.VOICE -> builder = WxMpXmlOutMessage.VOICE().mediaId(message.getMediaId());
            case WxConsts.XmlMsgType.VIDEO -> builder = WxMpXmlOutMessage.VIDEO().mediaId(message.getMediaId())
    ...
    default WxMpKefuMessage convert(MpMessageSendReqVO sendReqVO, MpUserDO user) {
        ...
        switch (sendReqVO.getType()) {
            case WxConsts.KefuMsgType.TEXT -> builder = WxMpKefuMessage.TEXT().content(sendReqVO.getContent());
            case WxConsts.KefuMsgType.IMAGE -> builder = WxMpKefuMessage.IMAGE().mediaId(sendReqVO.getMediaId());
            case WxConsts.KefuMsgType.VOICE -> builder = WxMpKefuMessage.VOICE().mediaId(sendReqVO.getMediaId());
            case WxConsts.KefuMsgType.VIDEO -> builder = WxMpKefuMessage.VIDEO().mediaId(sendReqVO.getMediaId())
