<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="商机标题" align="center" prop="businessTitle" />
      <el-table-column label="商机详情" align="center" prop="detail" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商机标题" prop="businessTitle">
          <el-input v-model="form.businessTitle" placeholder="请输入商机标题" />
        </el-form-item>
        <el-form-item label="商机详情" prop="detail">
          <el-input v-model="form.detail" placeholder="请输入商机详情" />
        </el-form-item>
        <el-form-item label="上报时间" prop="reportTime">
          <el-date-picker clearable v-model="form.reportTime" type="date" value-format="timestamp" placeholder="选择上报时间" />
        </el-form-item>
        <el-form-item label="跟进用户id" prop="followUserId">
          <el-input v-model="form.followUserId" placeholder="请输入跟进用户id" />
        </el-form-item>
        <el-form-item label="商机状态" prop="status">
...
<script>
import { createOpportunity, updateOpportunity, deleteOpportunity, getOpportunity, getOpportunityPage, exportOpportunityExcel } from "@/api/oa/opportunity";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        businessTitle: [{ required: true, message: "商机标题不能为空", trigger: "blur" }],
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getOpportunityPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createOpportunity(this.form).then(response => {
