    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['oa:project-impl:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['oa:project-impl:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['oa:project-impl:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['oa:project-impl:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createProjectImpl, updateProjectImpl, deleteProjectImpl, getProjectImpl, getProjectImplPage, exportProjectImplExcel } from "@/api/oa/projectImpl";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getProjectImplPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getProjectImpl(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateProjectImpl(this.form).then(response => {
        ...
        // 添加的提交
        createProjectImpl(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除工程实施列编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有工程实施列数据项?').then(() => {
          this.exportLoading = true;
          return exportProjectImplExcel(params);
        }).then(response => {
          this.$download.excel(response, '工程实施列.xls');
