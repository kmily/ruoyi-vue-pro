  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="发货状态(0代表未出售，1代表出售中，2代表已出售 )" prop="transferStatus">
        <el-input v-model="queryParams.transferStatus" placeholder="请输入发货状态(0代表未出售，1代表出售中，2代表已出售 )" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="出售用户名字" prop="sellingUserName">
        <el-input v-model="queryParams.sellingUserName" placeholder="请输入出售用户名字" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="出售用户头像" prop="sellingAvator">
        <el-input v-model="queryParams.sellingAvator" placeholder="请输入出售用户头像" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="出售用户id" prop="sellingUserId">
        <el-input v-model="queryParams.sellingUserId" placeholder="请输入出售用户id" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="openForm(undefined)"
                   v-hasPermi="['steam:other-selling:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['steam:other-selling:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="出售用户名字" align="center" prop="sellingUserName" />
      <el-table-column label="出售用户头像" align="center" prop="sellingAvator" />
      <el-table-column label="出售用户id" align="center" prop="sellingUserId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="openForm(scope.row.id)"
                     v-hasPermi="['steam:other-selling:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['steam:other-selling:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    <pagination v-show="total > 0" :total="total" :page.sync="queryParams.pageNo" :limit.sync="queryParams.pageSize"
                @pagination="getList"/>
    <!-- 对话框(添加 / 修改) -->
    <OtherSellingForm ref="formRef" @success="getList" />
    </div>
</template>

<script>
import * as OtherSellingApi from '@/api/steam/otherselling';
import OtherSellingForm from './OtherSellingForm.vue';
export default {
  name: "OtherSelling",
  components: {
          OtherSellingForm,
  ...
  methods: {
    ...
    async getList() {
      try {
      this.loading = true;
              const res = await OtherSellingApi.getOtherSellingPage(this.queryParams);
    ...
    async handleDelete(row) {
      const id = row.id;
      await this.$modal.confirm('是否确认删除其他平台在售编号为"' + id + '"的数据项?')
      try {
       await OtherSellingApi.deleteOtherSelling(id);
       await this.getList();
    ...
    /** 导出按钮操作 */
    async handleExport() {
      await this.$modal.confirm('是否确认导出所有其他平台在售数据项?');
      try {
        this.exportLoading = true;
        const res = await OtherSellingApi.exportOtherSellingExcel(this.queryParams);
        this.$download.excel(res, '其他平台在售.xls');
      } catch {
      } finally {
